...
public class ConvertUtils {
    ...
    private AlertDialog.Builder warining;

    public ConvertUtils(Context context) {
        this.context = context;
        warining = new AlertDialog.Builder(context);
        warining.setPositiveButton("Ok", null);
    }
    ...
    private void showWarning(String message, boolean hide) {
        warining.setMessage(message);
        if (!hide) {
            warining.show();
        }
    }

    private void showWarning(String message, int equals1, int equals2) {
        showWarning(message, equals1 != equals2);
    }
    ...
    public void setOutputFile() {
        ...
        showWarning("should have called setSource() first", mSourceResId != -1);
        sb.append('-');
        sb.append(mSourceResId);
        if (mCopyVideo) {
            showWarning("should have called setSize() first", mWidth != -1);
            showWarning("should have called setSize() first", mHeight != -1);
            sb.append('-');
            sb.append("video");
            sb.append('-');
            sb.append(mWidth);
    ...
    public void extractDecodeEditEncodeMux() throws Exception {
        ...
        try {
            if (mCopyVideo) {
                ...
                showWarning("missing video track in test video", videoInputTrack != -1);
...
//                audioTrack.setStereoVolume(0.2f, 0.2f);
                ...
                showWarning("missing audio track in test video", audioInputTrack != -1);
                MediaFormat inputFormat = audioExtractor.getTrackFormat(audioInputTrack);
                MediaFormat outputAudioFormat =
                        MediaFormat.createAudioFormat(
                                OUTPUT_AUDIO_MIME_TYPE, OUTPUT_AUDIO_SAMPLE_RATE_HZ,
                                OUTPUT_AUDIO_CHANNEL_COUNT);
                outputAudioFormat.setInteger(MediaFormat.KEY_CHANNEL_MASK, OUTPUT_AUDIO_BIT_RATE);
                ...
                for (String s: audioCodecInfo.getSupportedTypes()) {
                    Log.i("tag-----------", s);
                }
            }
    ...
    private void doExtractDecodeEditEncodeMux(
        ...
        final long startDurationExecute = System.currentTimeMillis();
            ...
            while (mCopyVideo && !videoEncoderDone
                ...
                if (encoderOutputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    if (outputVideoTrack >= 0) {
                        showWarning("video encoder changed its output format again?", true);
                    }
                    encoderOutputVideoFormat = videoEncoder.getOutputFormat();
                    break;
                }
                showWarning("should have added track before processing output", muxing);
                if (VERBOSE) {
            ...
            while (mCopyAudio && !audioEncoderDone
                ...
                if (encoderOutputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    if (outputAudioTrack >= 0) {
                        showWarning("audio encoder changed its output format again?", true);
                    }
        ...
        // Basic sanity checks.
        if (mCopyVideo) {
            showWarning("encoded and decoded video frame counts should match",
                    videoDecodedFrameCount, videoEncodedFrameCount);
            showWarning("decoded frame count should be less than extracted frame count",
                    videoDecodedFrameCount <= videoExtractedFrameCount);
        }
        if (mCopyAudio) {
            showWarning("no frame should be pending", -1, pendingAudioDecoderOutputBufferIndex);
        }
        // TODO: Check the generated output file.

        Log.d("tag-----------", "spend: " + (System.currentTimeMillis() - startDurationExecute));
    }
